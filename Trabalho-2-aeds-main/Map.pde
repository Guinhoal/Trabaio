class Map { // Classe Map representa o mapa do jogo, composto por v�rios chunks
  ...
  // M�todo para arrastar o mapa, ajustando o deslocamento
  void drag(float _offsetX, float _offsetY) {
    offsetX += _offsetX;
    offsetY += _offsetY;
  }

  // M�todo para converter a posi��o X da tela para a posi��o X da grade
  int gridPosX(int xScreen) {
    return floor((-offsetX + xScreen) / tileSize);
  }
  // M�todo para converter a posi��o Y da tela para a posi��o Y da grade
  int gridPosY(int yScreen) {
    return floor((-offsetY + yScreen) / tileSize);
  }

  //Faz o contr�rio do gridPosX
  int screenPosX(int gridX) {
    return (gridX * tileSize + (int)offsetX) + tileSize / 2;
  }

  //Faz o contr�rio do gridPosY
